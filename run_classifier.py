...
if __name__ == "__main__":
    ...
    for strategy_name in args.strategy:
        ...
        if args.ftrain < 1:
            ...
            for old_strategy_name in ["mHmatch", "pThigh"]:
                plot_outputs.old_strategy(ML_strategy, yhat_test_data[old_strategy_name] == 0, test_data, old_strategy_name)
                plot_outputs.confusion(ML_strategy, yhat_test_data[old_strategy_name] == 0, test_data, old_strategy_name)
            ...
            cPickle.dump({'yhat_test_ev': yhat_test_ev, 'yhat_mHmatch_test_ev': yhat_mHmatch_test_ev, 'yhat_pThigh_test_ev': yhat_pThigh_test_ev, 'y_event': y_event, 'mjb_event': mjb_event, 'pTj_event': pTj_event, 'w_test': w_test}, open(os.path.join(sample_name, 'for_event_performance_' + strategy_name + '.pkl'), 'wb'))
    ...
    if len(args.strategy) > 1 and (args.ftrain < 1):
        ...
        plot_roc.roc_comparison(sample_name)
