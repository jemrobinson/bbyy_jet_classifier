...
from bbyy_jet_classifier.plotting import plotinputs, plotoutputs, roc
...
if __name__ == "__main__":
    ...
    if args.ftrain > 0:
        ...
        plotinputs.input_distributions(ML_strategy, classification_variables, X_train, y_train, w_train, process="training")  # plot the feature distributions
        ...
        plotoutputs.classifier_output(ML_strategy, yhat_train, y_train, w_train, process="training", fileID=args.input.replace(".root", "").split("/")[-1])
    ...
    if args.ftrain < 1:
        ...
        plotinputs.input_distributions(ML_strategy, classification_variables, X_test, y_test, w_test, process="testing")
        ...
        plotoutputs.classifier_output(ML_strategy, yhat_test, y_test, w_test, process="testing", fileID=args.input.replace(".root", "").split("/")[-1])
        plotoutputs.old_strategy(ML_strategy.output_directory, mHmatch_test, y_test, w_test, "mHmatch")
        plotoutputs.old_strategy(ML_strategy.output_directory, pThigh_test, y_test, w_test, "pThigh")
