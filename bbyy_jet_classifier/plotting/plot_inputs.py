...
def input_distributions(classification_variables, training_data, test_data, output_directory, sample_name):
    ...
    -----
         ...
         training_data = dictionary, containing 'X', 'y', 'w' for the training set, where:
        ...
        test_data = dictionary, containing 'X', 'y', 'w' for the test set, where:
         ...
         output_directory = string, path to the folder where the pdf's will be saved
    ...
    for i, variable in enumerate(classification_variables):
        ...
        figure.savefig(os.path.join(output_directory, "{}_{}.pdf".format(variable, sample_name)))
        plt.close(figure)
