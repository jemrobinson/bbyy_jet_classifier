...
def plot_old_strategy(outdir, yhat_test, y_test, w_test, old_strategy_name):
	...
	_ = plt.hist(yhat_test[y_test == 1], weights=w_test[y_test == 1] / float(sum(w_test[y_test == 1])), 
		bins=np.linspace(0,1,10), histtype="stepfilled", label="Correct", color="blue", alpha=0.5)

	_ = plt.hist(yhat_test[y_test == 0], weights=w_test[y_test == 0] / float(sum(w_test[y_test == 0])), 
