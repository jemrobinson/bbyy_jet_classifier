...
class pThigh(BaseStrategy) :
  default_output_location = "output/pThigh"
  classifier_range = ( 0.0, 1.0 )


  def train_and_test( self, training_fraction ) :
    # -- Initialise testing
    logging.getLogger("pThigh::Train").info( "No training step needed, testing on all events..." )
    self.test_only()

    # -- Construct test events
    logging.getLogger("pThigh::Train").info( "Combining record arrays..." )
  ...
  def test_only( self ) :
    ...
    output_arrays = []
    logging.getLogger("pThigh::Test").info( "Calculating classifier output..." )
    for idx_array, _array in enumerate( [ self.correct_array, self.incorrect_array ] ) :
      output_arrays.append( rfn.append_fields( _array, ["pThigh","classID"], [_array.idx_by_pT == 0, np.full(_array.size,idx_array,int) ], dtypes=int, asrecarray=True ) )

    # -- Construct record arrays of correct/incorrect events
    logging.getLogger("pThigh::Test").info( "Constructing record arrays of correct/incorrect events..." )
